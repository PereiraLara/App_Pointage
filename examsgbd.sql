...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Jun 24, 2026 at 04:13 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `archiver_travailleur` (IN `p_id_travailleur` BIGINT, IN `p_date_fin` DATE)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `calculer_et_inserer_paques` (IN `p_annee` INT)   BEGIN
    ...
    DECLARE i INT; DECLARE k INT; DECLARE l INT; DECLARE m INT;
    ...
    -- Insérer TOUS les jours fériés mobiles valides pour cette année,
    -- en appliquant le décalage de chaque définition active
    INSERT IGNORE INTO jours_feries_mobiles (id_ferie, annee, date_ferie)
    SELECT fm.id_ferie, p_annee, DATE_ADD(date_paques, INTERVAL fm.decalage_jours DAY)
    FROM feries_mobiles fm
    WHERE YEAR(fm.date_debut) <= p_annee
      AND (fm.date_fin IS NULL OR YEAR(fm.date_fin) >= p_annee);
END$$
...
(35, 'test', 'testing', 5, 49, 31, '2026-06-24', '2026-06-24');
...
CREATE TRIGGER `trg_equipe_chef_update` AFTER UPDATE ON `equipe` FOR EACH ROW BEGIN
    ...
    -- CASE 1: the team's chef was changed
    IF NEW.chef_de_equipe <> OLD.chef_de_equipe THEN
        ...
        -- Insert/refresh new chef membership
        INSERT INTO travailleur_equipe (id_equipe, id_travailleur, role, date_debut, date_fin)
        VALUES (NEW.id_equipe, NEW.chef_de_equipe, 'chef', CURDATE(), NULL)
        ON DUPLICATE KEY UPDATE role = 'chef', date_debut = CURDATE(), date_fin = NULL;

        -- Close old chef's membership on this team
        UPDATE travailleur_equipe
        SET date_fin = CURRENT_DATE
        WHERE id_travailleur = OLD.chef_de_equipe
          AND id_equipe = NEW.id_equipe;

        -- Demote old chef if they lead no other ACTIVE team
        IF NOT EXISTS (
            SELECT 1 FROM equipe
            WHERE chef_de_equipe = OLD.chef_de_equipe
    ...
    -- CASE 2: the team was soft-deleted
...
(4, 'Vendredi Saint', -2, '2018-01-01', '2022-06-20', 0);

--
-- Triggers `feries_mobiles`
--
DROP TRIGGER IF EXISTS `trg_feries_mobiles_after_insert`;
DELIMITER $$
CREATE TRIGGER `trg_feries_mobiles_after_insert` AFTER INSERT ON `feries_mobiles` FOR EACH ROW BEGIN
    INSERT IGNORE INTO jours_feries_mobiles (id_ferie, annee, date_ferie)
    SELECT NEW.id_ferie, pa.annee,
           DATE_ADD(pa.date_paques, INTERVAL NEW.decalage_jours DAY)
    FROM paques_annuel pa
    WHERE YEAR(NEW.date_debut) <= pa.annee
      AND (NEW.date_fin IS NULL OR YEAR(NEW.date_fin) >= pa.annee);
END
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `trg_feries_mobiles_after_update`;
DELIMITER $$
CREATE TRIGGER `trg_feries_mobiles_after_update` AFTER UPDATE ON `feries_mobiles` FOR EACH ROW BEGIN
    -- Retirer les anciennes occurrences de ce férié
    DELETE FROM jours_feries_mobiles
    WHERE id_ferie = OLD.id_ferie;

    -- Réinsérer selon la nouvelle définition / fenêtre de validité
    INSERT IGNORE INTO jours_feries_mobiles (id_ferie, annee, date_ferie)
    SELECT NEW.id_ferie, pa.annee,
           DATE_ADD(pa.date_paques, INTERVAL NEW.decalage_jours DAY)
    FROM paques_annuel pa
...
(48, 6, 2026, NULL, NULL, NULL, NULL, NULL, '7', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
(1, 'P', 1.00, 'Prestation sur le chantier', '1971-03-16', NULL),
(2, 'C', 1.00, 'Congés payés', '1971-03-16', NULL),
(3, 'CC', 1.00, 'Congés de circonstance', '1978-07-03', NULL),
(4, 'CS', 0.00, 'Congé sans solde', '1978-07-03', NULL),
(5, 'M', 1.00, 'Congé maladie', '1978-07-03', NULL),
(6, 'MLD', 0.00, 'Congé maladie longue durée', '1978-07-03', NULL),
(7, 'CE', 0.00, 'Chômage économique', '1944-12-28', NULL),
(8, 'CI', 0.00, 'Chômage intempérie', '1969-06-27', NULL),
(9, 'AT', 1.00, 'Accident de travail', '1971-04-10', '2026-06-22'),
(10, 'R', 1.00, 'Récupération des heures supplémentaires', '1971-03-16', NULL),
(11, 'A', 0.00, 'Absence non justifiée', '1978-07-03', NULL),
(13, 'CPE', 1.00, 'Congé-éducation payé (formation interne)', '1985-01-01', '2004-12-31'),
(12, 'T', 1.00, 'Accident de trajet', '1971-04-10', '2015-06-30'),
(13, 'CPE', 1.00, 'Congé-éducation payé', '2005-01-01', '2022-06-30'),
(14, 'CHO', 0.00, 'Chômage temporaire complet', '1945-01-01', '1979-12-31'),
(9, 'AT', 2.00, 'Accident de travail', '2026-06-23', '2026-06-24'),
(9, 'ATs', 2.00, 'Accident de travail', '2026-06-25', '2026-06-28'),
(9, 'ATs', 2.00, 'Accident de travail', '2026-06-29', '2026-06-29'),
(9, 'ATs', 2.00, 'Accident de travail', '2026-06-30', '2026-07-11'),
(14, 'CHO', 0.00, 'Chômage temporaire complet', '1980-01-01', '1980-01-29'),
(14, 'CHO', 0.00, 'Chômage temporaire complet', '1980-01-30', NULL);
...
(2, 'admin', 'testing', '00.00.00-000.00', 'test@test.com', '$2y$10$KRuTDUICjANwXcQ5n13wnehH/2zNoeaEVt9dIDDBBtJNT0jy9NYRy', NULL),
...
(50, 'travailleur', 'test_travailleur', '11.11.11-111.11', 'travailleur@batipro.be', '$2y$10$QUZ7Cd3f/xXcd3/NspOrYOZg6Ql1qBztut3HKUiLaL8AfammVAvu2', NULL),
(49, 'chef_equipe', 'test_chef', '22.22.22-222.22', 'chef_equipe@batipro.be', '$2y$10$aCxuqTuDjVKowtZ8mmNTBuyY3ct2oyvyeHWGfWVCTBwmBJVBcqeru', NULL),
(51, 'contremaitre/manager', 'contremaitre', '33.33.33-333.33', 'contremaitre@batipro.be', '$2y$10$G7dhC8MKl/pUUtyy1CV6reAy/rpqMt6YJx3mJ0.l5I.XbXqtbJe4K', NULL);
...
(35, 49, 'chef', '2026-06-24', '2026-06-24'),
(35, 45, 'employe', '2026-06-24', '2026-06-24'),
(35, 48, 'employe', '2026-06-24', '2026-06-24');
...
(48, 50, '3/10 temps', '7.6', '2026-06-23', '2026-06-24'),
(48, 51, '2/3 temps', '7.6', '2026-06-25', '2026-06-30'),
(49, 52, 'temps plein', '8', '2026-06-27', NULL),
(50, 53, '1/3 temps', '7.6', '2026-06-26', NULL);
COMMIT;
